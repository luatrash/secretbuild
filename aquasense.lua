...
client.color_log(lua_color.r, lua_color.g, lua_color.b, " aquasense\0")
...
"Чтобы выйгрывать вы обязаны держать бренд aquasense'a",
"у твоей матери такая же узкая пизда как глаза d4ssh legend",
"ты такой же ебаный пес как  l4fn nolegend",
"блюхгенг луа вгетай хуйня)) - bit.ly/aquasenselua",
"тварь мб антипопадайки вгетаешь? - bit.ly/aquasenselua",
"конфиг тебе бы не помешал - bit.ly/aquasenselua",
...
"бля сори писька на пробел нажала а ты как бомж ебаный здох by aquasense",
...
"get aquasense.lua bozo =D - bit.ly/aquasenselua",
"ty4clip bot hhhh",
"take L or buy cfg - bit.ly/aquasenselua",
"why are you so ez?",
"b l u h g a n g  >  all",
"omfg you so bad ngl"
}
...
local menu = {
    ...
    visualsTab = {
        ...
        VisualsMultiselect = ui.new_multiselect(tab, container, "\a95b806ff› Select Visuals", "Crosshair Indicators", "aquasense menu", "Crosshair Arrows", "On screen logs"),
        ...
        hud = ui.new_combobox(tab, container, "aquasense menu", "Disabled", "v1", "v2"),
    ...
    GeneralTab = {
        newline1 = ui.new_label('AA', 'Anti-aimbot angles', '\n', 'string'),
        text1 = ui.new_label('AA', 'Anti-aimbot angles', 'aquasense.lua \a95b806ff[beta] build', 'string'),
...
local function clantag_set()
    ...
    if func.includes(ui.get(menu.visualsTab.MiscMultiselect), "Clantag") then
        ...
        if entity.get_prop(entity.get_game_rules(), "m_gamePhase") == 5 then
            clan_tag = clantag_anim('#aquasense.LUA', {13})
...
client.set_event_callback("paint", function()
    ...
    if readfile("bluhhudv1.png") ~= nil and ui.get(menu.visualsTab.hud) == "v1" and func.includes(ui.get(menu.visualsTab.VisualsMultiselect), "aquasense menu") then
        local png = images.load_png(readfile("bluhhudv1.png"))
        png:draw(ui.get(menu.visualsTab.hudX), ui.get(menu.visualsTab.hudY), 267, 172)
    end

    if readfile("bluhhudv2.png") ~= nil and ui.get(menu.visualsTab.hud) == "v2" and func.includes(ui.get(menu.visualsTab.VisualsMultiselect), "aquasense menu") then
    ...
    if ui.get(menu.visualsTab.indicators) == "Pixel" and func.includes(ui.get(menu.visualsTab.VisualsMultiselect), "Crosshair Indicators") then
        ...
        if ui.get(menu.visualsTab.indicators) == "Pixel" and func.includes(ui.get(menu.visualsTab.VisualsMultiselect), "Crosshair Indicators") then
            ...
            renderer.text(x / 2-28, y / 2 + 25, mr,mg,mb, 255, "-", 0, 'aquasense')
    ...
    if ui.get(menu.visualsTab.watermark) == "Default" then
        ...
        local output = ""
        local text_to_draw = "A Q U A S E N S E"
...
client.set_event_callback("paint_ui", function()
    ...
    ui.set_visible(menu.visualsTab.hud, isVisualsTab and isEnabled and func.includes(ui.get(menu.visualsTab.VisualsMultiselect), "aquasense menu"))
    ui.set_visible(menu.visualsTab.hudX, ui.get(menu.visualsTab.hud) ~= "Disabled" and isVisualsTab and isEnabled and func.includes(ui.get(menu.visualsTab.VisualsMultiselect), "aquasense menu"))
    ui.set_visible(menu.visualsTab.hudY, ui.get(menu.visualsTab.hud) ~= "Disabled" and isVisualsTab and isEnabled and func.includes(ui.get(menu.visualsTab.VisualsMultiselect), "aquasense menu"))
